...
entity CPU32bit is
	port(
		...
		PC_fetch_out_o, PC_decode_out_o, IR_fetch_out_o, immediate_decode_out_o, writeData_execute_out_o, rs1_execute_out_o, rs2_execute_out_o, 
		rd_execute_out_o, writeData_mem_out_o, newAddress_execute_out_o, writeData_wb_out_o,
		rs1_decode_out_o, rs2_decode_out_o: out std_logic_vector(31 downto 0);
		opcode_decode_out_o, func7_decode_out_o : out std_logic_vector(6 downto 0);
...
architecture structure of CPU32bit is
	...
	component execute
		port(
			...
			writeData, rs1_out, rs2_out, newAddress: out std_logic_vector(31 downto 0); --ALU inputs only used for testing
	...
	rd_execute_out, writeData_mem_out, newAddress_execute_out, writeData_wb_out,
	...
	begin
		...
		PC_out => PC_decode_out, jump_or_branch_flag => jump_or_branch_flag_execute_out);
		...
		writeData => writeData_execute_out, rs1 => rs1_decode_out, rs2 => rs2_decode_out, RAMen => RAMen_execute_out, RAMwe => RAMwe_execute_out,
		REGwe => REGwe_execute_out, rd_index_out => rd_index_execute_out, func3_out => func3_execute_out, 
		jump_or_branch_flag => jump_or_branch_flag_execute_out, newAddress => newAddress_execute_out, rs1_out => rs1_execute_out, 
		rs2_out => rs2_execute_out);
		...
		IR_fetch_out_o <= IR_fetch_out;
		immediate_decode_out_o <= immediate_decode_out;
		writeData_execute_out_o <= writeData_execute_out;
		rs1_execute_out_o <= rs1_execute_out;
		rs2_execute_out_o <= rs2_execute_out;
		rd_execute_out_o <= rd_execute_out;
		writeData_mem_out_o <= writeData_mem_out;
		newAddress_execute_out_o <= newAddress_execute_out;
		writeData_wb_out_o <= writeData_wb_out;
		rs1_decode_out_o <= rs1_decode_out;
		rs2_decode_out_o <= rs2_decode_out;
		opcode_decode_out_o  <= opcode_decode_out;
		func7_decode_out_o <= func7_decode_out;
		instructionType_decode_out_o <= instructionType_decode_out;
		func3_decode_out_o <= func3_decode_out;
		func3_execute_out_o <= func3_execute_out;
		rs1_index_decode_out_o <= rs1_index_decode_out;
		rs2_index_decode_out_o <= rs2_index_decode_out;
		rd_index_decode_out_o <= rd_index_decode_out;
		shamt_decode_out_o <= shamt_decode_out;
		rd_index_execute_out_o <= rd_index_execute_out;
		rd_index_mem_out_o <= rd_index_mem_out;
		rd_index_wb_out_o <= rd_index_wb_out;
		RAMen_execute_out_o <= RAMen_execute_out;
		RAMwe_execute_out_o <= RAMwe_execute_out;
		REGwe_execute_out_o <= REGwe_execute_out;
